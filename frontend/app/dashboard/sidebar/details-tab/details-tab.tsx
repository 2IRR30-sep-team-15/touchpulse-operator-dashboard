...
export default function DetailsTab({ profile }: { profile: Profile; }) {
    ...
    const fallbackInitials = profile.name.split(" ").map((n) => n[0]).join("");

    // Helper for rendering Yes/No badge
    const renderBooleanBadge = (value: boolean) => (
        <Badge 
            variant={value ? "default" : "secondary"} 
            className={`w-14 justify-center ${value ? 'bg-black text-white hover:bg-black/90' : 'bg-gray-200 text-gray-800 hover:bg-gray-300'}`}
        >
            {value ? "Yes" : "No"}
        </Badge>
    );
